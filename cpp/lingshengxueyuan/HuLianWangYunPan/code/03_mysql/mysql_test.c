...
int main()
{
	...
	//连接数据库
    MYSQL *conn = msql_conn("root", "123456", "lingsheng");
    if (conn == NULL) 
	{
        return 1;
    }
	...
	mysql_query(conn, "set names utf8");
	 ...
	 //插入
    sprintf(buf, "insert into %s (title,author,price,date) values ('%s', '%s', '%lf', '%s')", "book",
            "C++ Primer Plus(第6版)", "Stephen Prata", 95.00, "2012/07/01");
